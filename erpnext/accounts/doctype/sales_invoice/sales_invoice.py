...
class SalesInvoice(SellingController):
	...
	def validate_cai(self):
		cai = frappe.get_all("GCAI",["cai", "number", "name", "current_numbering", "due_date" ,"final_range"], filters = {"sucursal": self.sucursal, "pos_name": self.pos_name})
		...
		if number_remaining <= 20:
			showinfo('CAI', 'There are only {} available numbers'.format(number_remaining))
			#frappe.throw(_("There are only {} available numbers".format(number_remaining)))
		
		if number_remaining == 0:
			frappe.throw(_("The current numbering {} exceeds the limit numbering {}".format(cai[0].current_numbering, cai[0].final_range)))

		if date_now == str(cai[0].due_date):
			frappe.throw(_("There are 5 days left for your billing permit to expire."))
